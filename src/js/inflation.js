...
function inflate(org, cur, p, E, step) {
	...
	for(var i=0;i<cur.vertices.length;++i)
	{
		var diff = angles[i] - 2*Math.PI;
		var r = diff > 0 ? diff / (Math.PI / 2) : 0;	// diff > 0, red, saturated at 2.5 PI
		var g = 0;
		var b = diff < 0 ? (-diff) / (Math.PI / 2) : 0;	// diff > 0, red, saturated at 2.5 PI
		cur.colors[i] = new THREE.Color(r, g, b);
	}

	cur.colorsNeedUpdate = true;

	console.log('total displacement = ' + sum_moved + ' hyperbolic vertices = ' + result[1]);
}
